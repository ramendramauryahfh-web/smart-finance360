...
async function loadArticle() {
  ...
  try {
    ...
    // --- Insert Google AdSense In-Article Ads Dynamically ---
    insertInArticleAds();
...
// --- AUTO INSERT ADS IN ARTICLE ---
function insertInArticleAds() {
  const article = document.querySelector(".article-content");
  if (!article) return;

  const paragraphs = article.querySelectorAll("p");
  const adsClient = "ca-pub-9846205135944521";
  const adsSlot = "2835292413";

  let adsInserted = 0;
  const maxAds = 4;

  paragraphs.forEach((p, index) => {
    if ((index + 1) % 3 === 0 && adsInserted < maxAds) {

      const ad = document.createElement("div");
      ad.className = "my-4 text-center";
